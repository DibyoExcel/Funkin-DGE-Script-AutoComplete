...
function getLuaEngine(document?: vscode.TextDocument | undefined): string | undefined {
	...
	return vscode.workspace.getConfiguration().get("DGEVSCode.engine");
}
...
async function getOnlineData(file: string): Promise<any | undefined> {
	if (vscode.workspace.getConfiguration().get("DGEVSCode.offlineMode"))
		return undefined;

	const response = await needle("get", vscode.workspace.getConfiguration().get<string>("DGEVSCode.onlineDataURL") + file);
